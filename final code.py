...
mydb = mysql.connect(host = "localhost", user = "rootsd", passwd = "1234", database = "stfeeman")
...
def signup():
    ...
    mycursor.execute("INSERT INTO student_db (name, dob, ph, email_id, adm_no, password) VALUES (%s, %s, %s, %s, %s, %s)", get_val)
    mydb.commit()
...
def pay_check(name, adno, month):
    ...
    mycursor.execute("SELECT * FROM payment_db WHERE adm_no = %s AND month = %s", (adno, month))
    data = mycursor.fetchall()
    ...
    if len(data) == 0:
        ...
        while 1:
            ...
            if ch == "1":
                ...
                mycursor.execute("INSERT INTO payment_db (name, adm_no, amt, pay_stat, month) VALUES (%s, %s, %s, 'Paid', %s)", get_val)
                mydb.commit()               
                main()
            elif ch == "2":
                main()
